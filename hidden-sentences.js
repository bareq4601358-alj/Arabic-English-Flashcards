...
(() => {
  ...
  const REMOVED_KEY = "flash.ar-phrases.removed.v1";
  const CUSTOM_KEY = "flash.ar-phrases.customSentences.v1";
  ...
  const keyOf = (r) => norm(r.ar).toLowerCase();
  ...
  function buildHiddenList() {
    ...
    return keys.map((k) => {
      ...
      return { ar: k, en: "— (sentence not in bank)", tag: "hidden", _key: k };
    });
  }
