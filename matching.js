...
(() => {
  ...
  const STORAGE_TOPIC = "flash.ar-en.matchingTopic.v1";
  const STORAGE_CUSTOM = "flash.ar-en.customWords.v1";
  ...
  /** @type {{ ar: string, en: string, tag?: string }[]} */
  let fullBank = [];
  /** @type {Map<string, { ar: string, en: string, tag?: string }>} */
  ...
  function normalizeId(w) {
    ...
    const ar = String(w.ar || "").trim().toLowerCase();
    return `${ar}__${en}`;
  }
  ...
  function buildFullBank() {
    ...
    const merged = [...raw, ...custom]
      .filter((w) => w && typeof w.en === "string" && typeof w.ar === "string")
      .map((w) => ({
        en: norm(w.en),
        ar: norm(w.ar),
  ...
  function render() {
    ...
    for (let i = 0; i < ROUND_SIZE; i++) {
      ...
      leftFrag.appendChild(makeChoiceBtn("left", i, leftId, leftWord.ar));
      rightFrag.appendChild(makeChoiceBtn("right", i, rightId, primaryEn(rightWord.en)));
    }
