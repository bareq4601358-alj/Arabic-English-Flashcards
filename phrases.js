...
(() => {
  ...
  const STORAGE = {
    customSentences: "flash.ar-phrases.customSentences.v1",
    removed: "flash.ar-phrases.removed.v1",
    saved: "flash.ar-phrases.saved.v1",
  };
  ...
  const keyOf = (row) => norm(row.ar).toLowerCase();
  ...
  function loadCustomSentences() {
    ...
    return raw
      .map((r) => ({ ar: norm(r.ar), en: norm(r.en), tag: norm(r.tag || "") || "custom" }))
      .filter((r) => r.ar && r.en);
  }
  ...
  function loadSavedCards() {
    ...
    if (!Array.isArray(raw)) return [];
    return raw.map(normalizeSavedRow).filter((r) => r.ar && r.en);
  }
  ...
  function showCard(row) {
    ...
    if (!row) {
      els.textAr.textContent = "No sentences left in this deck.";
      els.textEnBack.textContent = "Restore some from Hidden, or clear your browser data to reset.";
